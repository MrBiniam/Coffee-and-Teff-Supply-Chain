...
export class DashboardComponent implements OnInit {
  ...
  private chart1() {
    try {
      ...
      const aaCoffeeData = (this.data?.dataAACoffee || []).map(item => this.ensureValidNumber(item?.price));
      const abCoffeeData = (this.data?.dataCoffeeAB || []).map(item => this.ensureValidNumber(item?.price));
      const cCoffeeData = (this.data?.dataCoffeeC || []).map(item => this.ensureValidNumber(item?.price));
      
      // Check if we have any valid data
      const hasAACoffeeData = aaCoffeeData.length > 0 && aaCoffeeData.some(d => d > 0);
      const hasABCoffeeData = abCoffeeData.length > 0 && abCoffeeData.some(d => d > 0);
      const hasCCoffeeData = cCoffeeData.length > 0 && cCoffeeData.some(d => d > 0);
      
      if (!hasAACoffeeData && !hasABCoffeeData && !hasCCoffeeData) {
        console.warn('No valid data for area chart, using defaults');
          ...
          try {
            ...
            if (order && order.product && Array.isArray(order.product) && order.product.length > 0) {
              ...
              if (product && product.seller === seller_id) {
                ...
                // Count based on status - Using partial string matching for case-insensitive comparison
                ...
                } else if (status.includes('ship') || status.includes('on_route') || status.includes('route')) {
