...
export class DeliveredOrderComponent implements OnInit {
  ...
  /**
   * Open dialog to rate the seller
   */
  rateSellerDialog(order: Order) {
    const dialogRef = this.dialog.open(DeliverOrderComponent, {
      data: {
        order: order,
        ratingTarget: 'seller',
        fromDeliveredPage: true // Indicate this is from the delivered page
      },
      width: '500px'
    });
  ...
  /**
   * Open dialog to rate the driver
   */
  rateDriverDialog(order: Order) {
    const dialogRef = this.dialog.open(DeliverOrderComponent, {
      data: {
        order: order,
        ratingTarget: 'driver',
  ...
  getBuyerImageUrl(username: string): string {
    return this.getUserImageUrl(username, 'buyer');
  }
